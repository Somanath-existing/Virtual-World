class Car {
   ...
   setSensorOptions(carOptions) {
      ...
      if (carOptions.type != "DUMMY") {
         ...
         // Pedestrian sensor - separate from car sensor
         this.pedestrianSensor = new Sensor(this, {
            rayCount: 15,
            rayLength: carOptions.sensorOptions.rayLength * 2,
            raySpread: Math.PI * 0.7,
            rayOffset: -Math.PI / 6,
         });
      }
   }
   ...
   update(
      ...
      if (!this.damaged) {
         ...
         if (this.sensor) {
            ...
            this.pedestrianSensor.update(this, pedestrianBorders);
            const pedestrianOffsets = this.pedestrianSensor.readings.map((s) =>
               s == null ? 0 : 1 - s.offset
            );
            
            // Combine car and pedestrian detections for the neural network
            lightOffsets.push(...carOffsets);
            lightOffsets.push(...pedestrianOffsets);
   ...
   draw(ctx, optimize = false) {
      ...
      if (this.pedestrianSensor && !optimize) {
         this.pedestrianSensor.draw(ctx, "orange", true);
      }
