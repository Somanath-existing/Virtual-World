...
function generateCars(N, markings, world) {
    const cars = [];
    let i = 0;
    while (i < N) {
        let spawnX, spawnY, alpha;
        
        // First 4 cars spawn at original marking positions
        if (i < markings.length) {
            const { center, directionVector } = markings[i];
            spawnX = center.x;
            spawnY = center.y;
            alpha = -angle(directionVector);
        } else {
            // Additional 6 cars spawn at random road segments
            const randomSegment = world.graph.segments[Math.floor(Math.random() * world.graph.segments.length)];
            const t = Math.random();
            spawnX = lerp(randomSegment.p1.x, randomSegment.p2.x, t);
            spawnY = lerp(randomSegment.p1.y, randomSegment.p2.y, t);
            const dir = subtract(randomSegment.p2, randomSegment.p1);
            alpha = -angle(dir);
        }
        
        cars.push(
            new Car(spawnX, spawnY, alpha, {
                ...defaultOptions,
...
function animate(time) {
    ...
    // Update traffic cars
    if (typeof traffic !== 'undefined' && traffic.length > 0) {
        for (let i = 0; i < traffic.length; i++) {
            if (traffic[i]) {
                traffic[i].update(world.roadBorders, []);
                traffic[i].polygon = traffic[i].createPolygon();
            }
        }
    }
    
    // Update pedestrians
    if (typeof pedestrians !== 'undefined' && pedestrians.length > 0) {
        for (let i = 0; i < pedestrians.length; i++) {
            pedestrians[i].update(cars);
        }
    }
    ...
    for (let i = 0; i < cars.length; i++) {
      ...
      if (cars[i].layer==null||changeLayer) {
        ...
        // Get road borders only for the car's segment and connected segments
        const relevantSegments = new Set();
        if (cars[i].segment) {
            relevantSegments.add(cars[i].segment);
            // Add directly connected segments
            for (const seg of world.graph.segments) {
                if (seg.includes(cars[i].segment.p1) || seg.includes(cars[i].segment.p2)) {
        ...
        const _borders = world
            ...
            .filter((b) => {
                // Only include borders from the car's segment or connected segments
                if (cars[i].segment && relevantSegments.size > 0) {
                ...
                // Fallback to layer filtering if no segment
                return b.layer == cars[i].layer;
            })
            .map((s) => [s.p1, s.p2]);
        ...
        // Original simple car detection - all nearby cars
        const carBorders = [];
        if (!optimizing) {
        ...
        // Separate pedestrian detection
        const pedestrianBorders = [];
        if (!optimizing && typeof pedestrians !== 'undefined' && pedestrians.length > 0) {
            for (let p = 0; p < pedestrians.length; p++) {
                const poly = pedestrians[p].getPolygon();
                if (poly.length > 0) {
                    pedestrianBorders.push([poly[0], poly[1]]);
                    pedestrianBorders.push([poly[1], poly[2]]);
                    pedestrianBorders.push([poly[2], poly[3]]);
                    pedestrianBorders.push([poly[3], poly[0]]);
                }
            }
        }
...
*/
    ...
    if (typeof traffic !== 'undefined' && traffic.length > 0) {
        for (let i = 0; i < traffic.length; i++) {
            if (traffic[i] && traffic[i].polygon) {
...
function giveAllPaths() {
    ...
    for (let i = cars.length - 1; i >= 0; i--) {
        // Assign targets cyclically to all cars
        ...
        // Only assign path if segment is valid
        if (cars[i].segment) {
            try {
                ...
                console.warn('Could not generate path for car', i, e);
                cars[i].assignedBorders = [];
                cars[i].destination = null;
            }
        } else {
            cars[i].assignedBorders = [];
            cars[i].destination = null;
